...
LevelLimits LevelToLevelLimits(uint8_t level) {
    ...
    default:
        std::cout << "Invalid codec level (" << static_cast<int>(level) << ")";
        return { 0, 0, 0, 0 };
    }
}
...
uint32_t H264ProfileLevelToMaxBR(VideoCodecProfile profile, uint8_t level) {
    ...
    default:
        std::cout << "Failed to query MaxBR for profile: ";
        return 0;
    }
}
...
bool CheckH264LevelLimits(VideoCodecProfile profile,
    ...
    if (bitrate > max_bitrate) {
        std::cout << "Target bitrate: " << bitrate << " exceeds Max: " << max_bitrate
            << " bit/s";
        return false;
    }

    if (framesize_in_mbs > H264LevelToMaxFS(level)) {
        std::cout << "Target frame size: " << framesize_in_mbs
    ...
    if (mbps > H264LevelToMaxMBPS(level)) {
        std::cout << "Target macroblock processing rate: " << mbps
