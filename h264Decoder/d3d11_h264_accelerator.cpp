...
H264Picture* D3D11H264Accelerator::CreateH264Picture() {
    D3D11PictureBuffer* picture = client_->GetPicture();
    if (!picture) {
        return nullptr;
    }
    return new D3D11H264Picture(picture);
}
...
DecoderStatus D3D11H264Accelerator::SubmitFrameMetadata(
    ...
    for (;;) {
        ...
        if (hr == E_PENDING || hr == D3DERR_WASSTILLDRAWING) {
            ...
            std::cout << "DecoderBeginFrame failed";
            return DecoderStatus::kFail;
        }
        else {
            break;
    ...
    sps_ = *sps;
    for (size_t i = 0; i < kRefFrameMaxCount; i++) {
    ...
    for (auto it = dpb.begin(); it != dpb.end(); i++, it++) {
        ...
        if (i >= kRefFrameMaxCount)
            return DecoderStatus::kFail;

        D3D11H264Picture* our_ref_pic = (*it)->AsD3D11H264Picture();
...
bool D3D11H264Accelerator::RetrieveBitstreamBuffer() {
    ...
    HRESULT hr = video_context_->GetDecoderBuffer(
        ...
        &buffer);
    if (!SUCCEEDED(hr)) {
        std::cout << "GetDecoderBuffer (Bitstream) failed" << std::endl;
        return false;
    }
...
void D3D11H264Accelerator::PicParamsFromSPS(DXVA_PicParams_H264* pic_param,
    ...
    SPS_TO_PP2(num_ref_frames, max_num_ref_frames);
    SPS_TO_PP2(wFrameWidthInMbsMinus1, pic_width_in_mbs_minus1);
    SPS_TO_PP2(wFrameHeightInMbsMinus1, pic_height_in_map_units_minus1);
    SPS_TO_PP2(residual_colour_transform_flag, separate_colour_plane_flag);
...
bool D3D11H264Accelerator::PicParamsFromPPS(DXVA_PicParams_H264* pic_param,
    ...
    PPS_TO_PP2(num_ref_idx_l0_active_minus1, num_ref_idx_l0_default_active_minus1);
    PPS_TO_PP2(num_ref_idx_l1_active_minus1, num_ref_idx_l1_default_active_minus1);
    PPS_TO_PP(entropy_coding_mode_flag);
    PPS_TO_PP2(pic_order_present_flag,
    ...
    if (pic_param->num_slice_groups_minus1) {
        ...
        std::cout << "num_slice_groups_minus1 == "
            << pic_param->num_slice_groups_minus1 << std::endl;
        return false;
    }
    return true;
}
...
DecoderStatus D3D11H264Accelerator::SubmitSlice(
    ...
    HRESULT hr = video_context_->GetDecoderBuffer(
        ...
        &buffer_size, &buffer);
    if (!SUCCEEDED(hr)) {
        std::cout << "GetDecoderBuffer (PictureParams) failed" << std::endl;
        return DecoderStatus::kFail;
    }
    ...
    hr = video_context_->ReleaseDecoderBuffer(
        video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_PICTURE_PARAMETERS);
    if (!SUCCEEDED(hr)) {
        std::cout << "ReleaseDecoderBuffer (PictureParams) failed" << std::endl;
        return DecoderStatus::kFail;
    }
    ...
    hr = video_context_->GetDecoderBuffer(
        ...
        &buffer);
    if (!SUCCEEDED(hr)) {
        std::cout << "GetDecoderBuffer (QuantMatrix) failed" << std::endl;
        return DecoderStatus::kFail;
    }
    ...
    if (!SUCCEEDED(hr)) {
        std::cout << "ReleaseDecoderBuffer (QuantMatrix) failed" << std::endl;
        return DecoderStatus::kFail;
    }
...
bool D3D11H264Accelerator::SubmitSliceData() {
    ...
    HRESULT hr = video_context_->GetDecoderBuffer(
        ...
        &buffer_size, &buffer);
    if (!SUCCEEDED(hr)) {
        std::cout << "GetDecoderBuffer (SliceControl) failed" << std::endl;
        return false;
    }
    ...
    hr = video_context_->ReleaseDecoderBuffer(
        video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_SLICE_CONTROL);
    if (!SUCCEEDED(hr)) {
        std::cout << "ReleaseDecoderBuffer (SliceControl) failed" << std::endl;
        return false;
    }
    ...
    if (!SUCCEEDED(hr)) {
        std::cout << "ReleaseDecoderBuffer (BitStream) failed" << std::endl;
        return false;
    }
    ...
    subsamples_.clear();
    if (!SUCCEEDED(hr)) {
        std::cout << "SubmitDecoderBuffers failed" << std::endl;
        return false;
    }
...
DecoderStatus D3D11H264Accelerator::SubmitDecode(
    ...
    if (!SUCCEEDED(hr)) {
        std::cout << "DecoderEndFrame failed" << std::endl;
        return DecoderStatus::kFail;
    }
