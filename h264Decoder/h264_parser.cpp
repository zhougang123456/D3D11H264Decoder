...
Rect H264SPS::GetVisibleRect() const {
    Size coded_size = GetCodedSize();
    if (coded_size.IsEmpty())
        return Rect(0, 0, 0, 0);

    if (!frame_cropping_flag)
        return Rect(coded_size);
...
VideoColorSpace H264SPS::GetColorSpace() const {
    if (colour_description_present_flag) {
        return VideoColorSpace(
            ...
            video_full_range_flag ? ColorSpace::RangeID::FULL
...
bool H264Parser::ParseNALUs(const uint8_t* stream,
    ...
    while (true) {
        ...
        else if (result == H264Parser::kEOStream) {
            return true;
        }
        else {
...
H264Parser::Result H264Parser::ParseSPS(int* sps_id) {
    ...
    *sps_id = -1;
    ...
    if (sps->pic_order_cnt_type == 0) {
        ...
        if (offset_acc < 0 || offset_acc > INT_MAX / 2)
            return kInvalidStream;
...
H264Parser::Result H264Parser::ParseSPSScalingLists(H264SPS* sps) {
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (seq_scaling_list_present_flag) {
            res = ParseScalingList(sizeof(sps->scaling_list4x4[i]) / sizeof(int),
    ...
    for (int i = 0; i < ((sps->chroma_format_idc != 3) ? 2 : 6); ++i) {
        ...
        if (seq_scaling_list_present_flag) {
            res = ParseScalingList(sizeof(sps->scaling_list8x8[i]) / sizeof(int),
...
H264Parser::Result H264Parser::ParsePPSScalingLists(const H264SPS& sps,
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (pic_scaling_list_present_flag) {
            res = ParseScalingList(sizeof(pps->scaling_list4x4[i]) / sizeof(int),
    ...
    if (pps->transform_8x8_mode_flag) {
        for (int i = 0; i < ((sps.chroma_format_idc != 3) ? 2 : 6); ++i) {
            ...
            if (pic_scaling_list_present_flag) {
                res = ParseScalingList(sizeof(pps->scaling_list8x8[i]) / sizeof(int),
...
H264Parser::Result H264Parser::ParseVUIParameters(H264SPS* sps) {
    ...
    if (aspect_ratio_info_present_flag) {
        ...
        if (aspect_ratio_idc == H264SPS::kExtendedSar) {
            ...
            const int max_aspect_ratio_idc = sizeof(kTableSarWidth) / sizeof(int) - 1;
...
H264Parser::Result H264Parser::ParseDecRefPicMarking(H264SliceHeader* shdr) {
    ...
    if (shdr->idr_pic_flag) {
        ...
        if (shdr->adaptive_ref_pic_marking_mode_flag) {
            size_t i;
            for (i = 0; i < sizeof(shdr->ref_pic_marking) / sizeof(H264DecRefPicMarking); ++i) {
            ...
            if (i == sizeof(shdr->ref_pic_marking) / sizeof(H264DecRefPicMarking)) {
